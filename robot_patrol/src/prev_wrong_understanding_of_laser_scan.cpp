...
float radian_from_scan_index(int scan_index) {
  ...
  if (659 >= scan_index && scan_index >= 495) {
    rad = (scan_index - 659) / 164 * (pi / 2);
  } else if (164 >= scan_index && scan_index >= 0) {
    rad = (scan_index / 164) * (pi / 2);
  }
  return rad;
};
...
private:
  ...
  void laser_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    float interested_max = 200; // prev good value 20
    float tolerated_min =
    ...
    for (auto itaa = aggregation_of_bands.begin();
      ...
      RCLCPP_INFO(this->get_logger(),
                  "found a band (%d,%d)with max:value %f, maximum min:value "
                  "%f, min:value%f",
                  std::get<0>(boundary_aband), std::get<1>(boundary_aband),
                  c_band.get_statistic_max(), cur_maxfar,
                  c_band.get_statistic_min());
    ...
    if (band_direction > 0)
      assert(abs(std::min(msg->ranges[band_direction], interested_max) -
                 band_value) < 0.001);
    if (band_value <= endanged_min) {
      if (std::abs(band_direction - raw_avoid_direction) <= avoid_gap) {

        if (std::abs(659 - raw_avoid_direction) <= turn_around_gap) {
          direction_ = 659 - (raw_avoid_direction +
          ...
          if ((659 - raw_avoid_direction) < raw_avoid_direction) {
    ...
    ling.angular.z = radian_from_scan_index(direction_) * 0.5;
    RCLCPP_INFO(this->get_logger(),
